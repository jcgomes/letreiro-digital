...
class frm_consulta():
    def __init__(self):
        ...
        self.frm_consulta.set_title("Consulta de Letras") # CONFIGURAR

        # =======================================================================================
        self.ls_tv_consulta = gtk.ListStore(str, str, str) # CONFIGURAR (deve ter quantidade de campos  igual a quantidade de items do tv_colunas)
        self.tv_consulta = app.builder.get_object("tv_consulta")
        self.tv_consulta.set_model(self.ls_tv_consulta)
        # =======================================================================================

        # =============================================================================================
        tv_colunas = {'0':'ID',  '1':'Título',  '2':'Autor'} # CONFIGURAR  (formato: 'numero_sequencial':'texto_a_ser_exibido' )
        ...
        # ======================================================================================
        cb_campos = {'letras.id':'ID',  'letras.titulo':'Título',  'autores.autor':'Autor'} # CONFIGURAR (formato: 'campo_tabela':'texto_a_ser_exibido')
        for cb_campo in sorted(cb_campos):
            self.ls_cb_parametro.append([str(cb_campo), cb_campos[str(cb_campo)]])
        self.cb_parametro.set_active(2) # CONFIGURAR
        # ======================================================================================
    ...
    def consultar(self,  orderby):
        try:
            ...
            self.sql_select = 'SELECT letras.id, letras.titulo, autores.autor FROM letras LEFT OUTER JOIN autores ON letras.autor = autores.id WHERE '  +str(self.cb_parametro.get_active_text())+ ' LIKE ? ORDER BY '+orderby
            ...
            if self.rs_select != []: # Se a tabela não estiver vazia então prossegue
                for self.rec_select in self.rs_select:
                    self.ls_tv_consulta.append([self.rec_select[0],self.rec_select[1],self.rec_select[2]])
                self.con.close()
...
class frm_cadastro():
    def __init__(self):
        app.builder.add_from_file("ui/letras/cadastro.glade")
        self.frm_cadastro = app.builder.get_object("frm_cadastro")
        self.e_titulo = app.builder.get_object("e_titulo")
        self.ls_autor = gtk.ListStore(str, str)
        self.cb_autor = app.builder.get_object("cb_autor")
        self.cb_autor.set_model(self.ls_autor)
        self.tv_letra = app.builder.get_object("tv_letra")
        self.tb_letra = gtk.TextBuffer()
        self.tb_letra = self.tv_letra.get_buffer()
        self.tb_letra.connect("changed", self.on_tb_letra_changed)
        self.lbl_caracteres = app.builder.get_object("lbl_caracteres")
        app.builder.connect_signals(self)
        ...
        if db.resultado_consulta > 0:
            ...
            self.limpa_objetos()
            self.con = sqlite3.connect(config.string_db.db)
            self.cur_autores = self.con.cursor()
            self.cur_autores.execute('SELECT id,autor FROM autores')
            self.rs_autores = self.cur_autores.fetchall()
            for self.rec_autores in self.rs_autores:
                self.ls_autor.append([self.rec_autores[0], self.rec_autores[1]])
            self.con.close()


    def limpa_objetos(self):
        self.e_titulo.set_text("")
        self.cb_autor.set_active(-1)
        self.tb_letra.set_text("")
        self.e_titulo.grab_focus()
        db.resultado_consulta = 0
        db.text_tag = 0
        db.alterado = False
    ...
    def get_registros(self):
        try:
            ...
            self.cur_letras = self.con.cursor()
            self.cur_letras.execute('select id, autor, titulo, letra from letras where id=?', self.parametro)
            self.rs_letras = self.cur_letras.fetchall()

            for self.rec_letras in self.rs_letras:
                self.e_titulo.set_text(self.rec_letras[2])
                self.tb_letra.set_text(self.rec_letras[3])
                self.contador = len(self.rec_letras[3])
                self.lbl_caracteres.set_text(str(int(450)-int(self.contador))+' caracteres restantes de um total de 450 caracteres.')
            ...
            self.ativo = 0
            for self.rec_autores in self.rs_autores:
                self.contador = self.contador+1 # inicia no zero (que é o primeiro numero do index do combobox)
                if self.rec_autores[0] == self.rec_letras[1]:
                    self.ativo = self.contador
                self.ls_autor.append([self.rec_autores[0], self.rec_autores[1]])
            self.cb_autor.set_active(int(self.ativo))
    ...
    def on_aplicar(self, tipo):
        try:
            self.autor = self.cb_autor.get_active_text()
            self.titulo = self.e_titulo.get_text()
            self.letra = self.tb_letra.get_text(self.tb_letra.get_start_iter(), self.tb_letra.get_end_iter(), include_hidden_chars=True)

            reload(config) # atualiza as informações do módulo.
            self.con = sqlite3.connect(config.string_db.db)
            self.parametro = [self.autor, unicode(self.titulo,  'utf-8'),  unicode(self.letra, 'utf-8'),  str(db.resultado_consulta), ]
            if tipo == 'update':
                self.cur_update = self.con.cursor()
                self.cur_update.execute('UPDATE letras SET autor=?, titulo=?, letra=? WHERE id=?', (self.parametro[0],self.parametro[1],self.parametro[2],self.parametro[3]))
                self.con.commit()
            else:
                self.cur_insert = self.con.cursor()
                self.cur_insert.execute('INSERT INTO letras (autor, titulo, letra) VALUES (?,?,?)', (self.parametro[0],self.parametro[1], self.parametro[2]))
                self.con.commit()
                ...
                self.sql_select = "SELECT MAX(id) FROM letras"
                self.cur_select = self.con.cursor()
    ...
    def on_e_titulo_changed(self, widget):
        db.alterado = True

    def on_cb_autor_changed(self, widget):
        db.alterado = True

    def on_tb_letra_changed(self, widget):
        db.alterado = True
        self.contador = len(self.tb_letra.get_text(self.tb_letra.get_start_iter(), self.tb_letra.get_end_iter(), include_hidden_chars=False))
        self.lbl_caracteres.set_text(str(int(450)-int(self.contador))+' caracteres restantes de um total de 450 caracteres.')
